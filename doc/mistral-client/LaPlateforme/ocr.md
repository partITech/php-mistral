# OCR with Document URL using Mistral

This example demonstrates how to perform **OCR (Optical Character Recognition)** on a document using a public URL with the Mistral API. The extracted text is returned along with the option to include base64-encoded images of the document pages.

> [!TIP]
> This example utilizes the **Document Capabilities** of Mistral, allowing you to process structured documents like PDFs and images. More details can be found in the [official Mistral documentation](https://docs.mistral.ai/capabilities/document/).

## Example: OCR from a Document URL
...
// Prepare the document message using a public URL (e.g., a PDF)
$messages = $client
                ->getMessages()
                ->addDocumentMessage(
...
try {
    $result = $client->ocr(
        ...
        [
            ...
            'include_image_base64' => true        // Optionally include page images as base64
        ]
    );

    print_r($result->getPages());                 // Output the OCR results page by page
} catch (\Throwable $e) {
    echo $e->getMessage();
    exit(1);
}
...
|------------------------|---------|------------------------------------------------------------------------------------------------------|
| `model`                | string  | The OCR model to use. For Mistral, use `mistral-ocr-latest`.                                         |
| `include_image_base64` | boolean | If `true`, the response includes base64-encoded images of each page alongside the extracted content. |

> [!NOTE]
> The **Mistral OCR model** can process various document formats (PDFs, images, etc.) and extract text along with layout information.
...
> [!CAUTION]
> Large documents with many pages or high-resolution images might result in large payloads if `include_image_base64` is enabled.

## Supported Document Types

- PDF
- JPEG, PNG, TIFF
- Other formats supported by the Mistral API

> [!IMPORTANT]
> Ensure that the document URL is publicly accessible if you are using the **DOCUMENT_URL** type.
